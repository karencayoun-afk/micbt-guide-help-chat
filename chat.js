...
function loadFAQDatabase() {
  ...
  try {
    // Try multiple paths — Netlify functions run from unpredictable cwd
    const candidates = [
      path.join(__dirname, 'micbt_faq_database.xml'),           // same dir as function
      path.join(__dirname, '..', 'micbt_faq_database.xml'),     // one level up
      path.join(__dirname, '..', '..', 'micbt_faq_database.xml'), // two levels up
      path.join(process.cwd(), 'micbt_faq_database.xml'),       // cwd fallback
    ];
    ...
    if (!xml) {
      console.warn('FAQ database not found. Tried:', candidates);
      return [];
    }
    ...
    faqCache = faqs;
    console.log('FAQ database loaded from ' + usedPath + ': ' + faqs.length + ' entries');
    return faqs;
  } catch (err) {
...
function friendlyError(type) {
  const map = {
    overloaded_error:     "I am a little busy right now -- please try again in a moment.",
    rate_limit_error:     "I am a little busy right now -- please try again in a moment.",
    context_length_error: "That conversation has grown quite long. Try starting a fresh session.",
    authentication_error: "There is a configuration issue. Please contact support@mindfulness.net.au.",
